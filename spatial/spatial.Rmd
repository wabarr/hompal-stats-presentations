...
Add an annotation to the plot with a label for the USA. Note: you can try using `geom_sf_label` or just `annotate`, which might be simpler.
...
ggplot(data=world) + 
  ...
  scale_fill_viridis_c() + 
  coord_sf(expand=F) + #don't expand the map to an even number
  annotate(geom="text", x=-99, y=40, label="USA", color="white") + 
  labs(x=NULL, y=NULL)
```
