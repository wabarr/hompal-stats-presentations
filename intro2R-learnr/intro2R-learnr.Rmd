...
```{r femurmake, exercise=T, exercise.eval=T}
femur <- c(450.8,444.8,415.1,487.3,349,459.5,431.7,454.1,420.6,387.2)
...
Notice that after I created the vectors using the `c()` function, I have typed the name of the each vector (i.e. `femur` or `tibia`). If I didn't include those lines, then R would have silently created the vectors, without printing the values to the screen.  When you type the name of a variable in R and run that code, R will print the current value of the requested variable. 
...
We can use the `head()` function to get a glimpse of the dataset.  By default, `head()` shows the first few rows of a data frame.  
